...
# ─────────────────────────────────────────────────────────────────────────────
# Phase 21: PWA push notifications
# ─────────────────────────────────────────────────────────────────────────────
 
class PushSubscription(Base):
    """
    One row per browser/device push subscription (Web Push API). A user
    can have several rows at once -- Athena open on a phone AND a laptop
    both register separately, and both should receive a reminder push.
 
    `endpoint` is the push-service URL the browser vendor assigns to this
    specific subscription (unique per browser install) -- it doubles as
    the natural dedupe key, since re-subscribing on the same device with
    the same browser reuses the same endpoint. `p256dh` and `auth` are
    the subscription's encryption keys, required to encrypt the push
    payload per the Web Push protocol (RFC 8291) -- both come straight
    from `PushSubscription.toJSON().keys` in the browser.
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# Phase 23: Intelligence Layer — Proactive Insights
# ─────────────────────────────────────────────────────────────────────────────
 
class ProactiveInsight(Base):
    """
    One row per unprompted nudge Athena decided to surface (core/proactive_engine.py).
    Unlike Reminder/ReminderFired (which fire because the user explicitly asked to be
    reminded of something), these are generated by the background engine deciding,
    from the user's own context (goals, calendar, overdue reminders, recent
    conversation), that something is worth mentioning without being asked.
 
    Persisted (not just pushed) so:
      - the frontend can show a dismissible feed of recent insights, not just a
        one-shot OS notification that's easy to miss
      - the engine can rate-limit itself per user by checking `created_at` of the
        most recent row before deciding to generate another one
    """
    __tablename__ = "proactive_insights"
 
    id          = Column(Integer, primary_key=True, index=True)
    user_id     = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    kind        = Column(String, nullable=False, default="general")  # reminder_upcoming | calendar_soon | goal_stale | pattern | general
    message     = Column(Text, nullable=False)
    source_ref  = Column(String, nullable=True)  # optional free-form pointer, e.g. "reminder:42"
